...
class ImportConvert {
	...
	function processLine($source, $data, $arr639_1, $arr639_2) {
		...
		$metas = array('dc_type', 'bpi_cote', 'bpi_theme', 'dc_subject');
		
		$urls = explode('@;@', $data['bpi_dm_lien_url']);
		
		$ndx=0;
		foreach ($urls as $url_str) {
			
			$url = parse_url($url_str);
			if (count($url)==1) continue;
			
			$name = $this->getValueOcc($data['bpi_dm_title'], $ndx);
			if (empty($name)) $name = $this->getValueOcc($data['dc_title'], $ndx);
			if (empty($name)) $name = '<sans nom>';
			...
			$lan1 = $this->getValueOcc($data['dc_language'], $ndx);
			...
			foreach ($metas as $meta) {
				$metadata .= 'bpi_' . $meta . ':' . preg_replace( "/\r|\n/", " ",$this->getValueOcc($data[$meta], $ndx)) . "\n";
			}
...
// $converter = new ImportConvert('/Users/bejean/Desktop/BPI/BPI-small.csv');
